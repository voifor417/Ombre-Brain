...
- `/toy` 页面 `vib:M,S` 改用真机校准确认的 SL278B 原生帧 `55 03 00 00 [模式] [强度] 00`（模式5=波浪，强度0-10有效）；开源 Iker 文档帧 `55 03 03 00 M S` 经真机逐项排除（原样/补尾/拉满均无响应），废弃。
- `suk:M,S`、`thr:Z` 与伸缩停止帧统一为原生 7 字节帧型（`55 09 00 00 M S 00` / `55 07 00 00 Z 00 00`），待下次真机校准验证。
- `toy` MCP 工具说明同步校准结论，注明 `vN` 等价于 `vib:1,N`。

### 安全 / Safety

- toy 双层死人开关：服务器端非 stop 指令 10 分钟未刷新，`GET /api/toy/command` 自动返回 stop（防控制端窗口中断后设备无人喊停持续运行，2026-07-18 真实事故）；`/toy` 页面端与服务器失联超 90 秒，看门狗本地自动全停（防断网后 keepalive 无限重发）。

### 维护 / Chores

- VERSION + `src/VERSION` -> 2.4.23（src/VERSION 自 2.4.18 起漏更，一并对齐）。
